package TestCases.APITests.listemployeeassignedtoprogram;
...
public class ListAllEmployeeAssignedToProgramTest extends base {

    @Test
    public void verify_list_all_employee_assigned_to_program_with_valid_token (){
        ...
        String  update_file = builder.updateRequestObject("listemployeeassignedtoprogram/list_employee_assigned_to_program_with_valid_token",pathParameter, queryParameters, header, body,body);
        JSONObject get_program_response=builder.performRequest(update_file);

        ResponseValidator assignemployee_validator = new ResponseValidator(get_program_response);
        assignemployee_validator.statusCodeShouldBe(200);

        String startdate_ex = assignemployee_validator.getDataFromBody("$.data[*].start_date");
        assertThat(startdate_ex).isNotBlank();

        String duedate_ex = assignemployee_validator.getDataFromBody("$.data[*].due_date");
        assertThat(duedate_ex).isNotBlank();

        String employeeid_ex = assignemployee_validator.getDataFromBody("$.data[*].employee_id");
        assertThat(employeeid_ex).isNotBlank();

        String programeid_ex = assignemployee_validator.getDataFromBody("$.data[*].program_id");
        assertThat(programeid_ex).isNotBlank();

        String status_ex = assignemployee_validator.getDataFromBody("$.data[*].status");
        assertThat(status_ex).isNotBlank();

        String completeddatetime_ex = assignemployee_validator.getDataFromBody("$.data[*].completed_datetime");
        assertThat(completeddatetime_ex).isNotBlank();

        String approvaldatetime_ex = assignemployee_validator.getDataFromBody("$.data[*].approval_datetime");
        assertThat(approvaldatetime_ex).isNotBlank();
    }
    @Test
    public void verify_list_all_employee_assigned_to_program_with_blank_token () {
        RequestBuilder builder = new RequestBuilder();

        JSONObject response = builder.performRequest("listemployeeassignedtoprogram/list_employee_assigned_to_program_with_valid_token");
    ...
    @Test
    public void verify_list_all_employee_assigned_to_program_with_invalid_token() {
        RequestBuilder builder = new RequestBuilder();
        ...
        String  update_file = builder.updateRequestObject("listemployeeassignedtoprogram/list_employee_assigned_to_program_with_invalid_token",pathParameter, queryParameters, header, body,body);
